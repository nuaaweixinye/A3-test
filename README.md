...
| ④ 智能辅导（加分项） | ✅ | `lib/agents/tutor-agent.ts`、`app/tutor`（多轮答疑+朗读） |
| ⑤ 学习效果评估（加分项） | ✅ | `lib/agents/eval-agent.ts`、`app/eval`（雷达+回写画像+路径调整） |
...
```
镜像为多阶段构建 + Next.js standalone 输出，内含 `knowledge_base/`，`node server.js` 直接运行。
...
- [x] 防幻觉第 3 层事实核查（`lib/knowledge/fact-check.ts`）
