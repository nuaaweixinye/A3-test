...
| 赛题功能 | 状态 | 实现位置 |
| --- | --- | --- |
| ① 对话式学习画像构建（≥6 维度） | ✅ | `backend/agents/profile-agent.ts` |
| ② 多智能体协同资源生成（6 类并行） | ✅ 全部 | doc/quiz/mindmap/video/code/reading Agent，LangGraph 并行分发 |
| ③ 个性化学习路径规划与推送 | ✅ | `backend/agents/planner-agent.ts` |
| ④ 智能辅导（加分项） | ✅ | `backend/agents/tutor-agent.ts`、`app/tutor`（多轮答疑+朗读） |
| ⑤ 学习效果评估（加分项） | ✅ | `backend/agents/eval-agent.ts`、`app/eval`（雷达+回写画像+路径调整） |
| 防幻觉 + 内容安全 | ✅ 四层完整 | 检索约束 / Prompt 约束 / **事实核查(第3层)** / 引用标注 |

> **学习闭环已打通**：画像 → 路径 → 6 资源生成 → 学习(自评+浏览) → ④辅导 → ⑤评估 → **回写画像 + 动态调整路径**。
...
# 透传星火密钥：export SPARK_API_KEY=... 后再 up；未设置则 mock 模式
```
镜像为多阶段构建 + Next.js standalone 输出，内含 `backend/backend/knowledge_base/`，`node server.js` 直接运行。

## 多智能体协同框架

`backend/graph.ts` 用 **LangGraph StateGraph** 编排 8 个角色智能体，共享状态在节点间流转：
...
1. **检索约束**：所有生成前先检索知识库，作为 Prompt 上下文（`backend/knowledge/retriever.ts`）。
2. **Prompt 约束**：System Prompt 强制"仅基于知识库内容生成，不得编造"。
3. **事实核查**：生成后交叉验证（`backend/knowledge/fact-check.ts`），给出 score / flagged。
4. **引用标注**：文档标注来源（`sources`），便于评审验证。

## 目录结构

```
backend/                       后端代码（全部服务端逻辑）
  ...
  knowledge/fact-check.ts      防幻觉第 3 层事实核查
  backend/knowledge_base/              课程知识库（数据结构与算法，8 篇种子文档）
  types/index.ts               前后端共享类型契约

frontend/                      前端代码（全部客户端逻辑）
  components/
    chat/ChatPanel.tsx         对话面板（驱动闭环、解析 SSE、资源进度）
    profile/ProfileRadar.tsx   ECharts 6 轴雷达图
    resource/                  资源卡片 + Markdown 渲染 + 语音播放
    eval/EvalRadar.tsx         评估雷达图
  lib/
    store/useLearningStore.ts  Zustand 全局状态
    sse-client.ts              浏览器 SSE 消费

app/                           Next.js App Router（薄壳：路由分发）
  api/learn/route.ts           SSE 流式闭环接口（画像→路径→6 资源并行）
  api/tutor/route.ts           SSE 流式辅导接口
  api/eval/route.ts            JSON 评估接口
  page.tsx                     首页：对话入口
  profile/page.tsx             画像雷达 + 6 维度详情
  learn/page.tsx               学习路径时间线 + 资源画廊
  tutor/page.tsx               辅导多轮对话
  eval/page.tsx                评估看板
```
...
- [x] 加分项 ④ 智能辅导（多轮答疑 + Web Speech 朗读）
- [x] 加分项 ⑤ 学习效果评估（雷达 + 回写画像 + 动态路径调整）
- [x] 防幻觉第 3 层事实核查（`backend/knowledge/fact-check.ts`）
- [x] Docker 一键部署（`docker compose up --build`，standalone 输出）
- [x] 配套文档（`docs/`：开发说明书 / 测试说明书 / 开源标注与AI工具 / 架构图）
- [ ] 接入讯飞 TTS 替换浏览器占位语音（辅导/视频）
- [ ] RAG 升级为 Chroma + BGE-M3 向量检索
- [ ] 画像持久化（PostgreSQL/SQLite）、会话缓存（Redis）
- [ ] 演示视频、演示 PPT
